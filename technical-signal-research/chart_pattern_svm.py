# ============================================================
# NAMED CHART PATTERN ML TRADING FRAMEWORK
# SVM + RBF Kernel
...
#
# See chart_pattern_svm_tradingview_style.py for the same pipeline with
# TradingView-style dark candlestick chart output instead of plain line charts.
# ============================================================
...
def save_or_show_plot(filepath, show_plots=True):
    plt.tight_layout()
...
def plot_equity_curves(train_bt, test_bt, ticker, output_prefix, show_plots=True):
...
def plot_pattern_events(test_bt, ticker, output_prefix, show_plots=True):
    plt.figure(figsize=(15, 6))
...
def plot_individual_pattern_examples(data, ticker, output_prefix, show_plots=True, max_examples=5):
    """
    For each named pattern, save one full-size image per example occurrence.
    Files are named: {ticker}_pattern_{PatternName}_example_{n}.png
    """

    patterns = {
        "Double_Bottom":            ("double_bottom_flag",           "bullish", "^", "green"),
        "Double_Top":               ("double_top_flag",              "bearish", "v", "red"),
        "Head_and_Shoulders":       ("head_shoulders_flag",          "bearish", "v", "red"),
        "Inverse_Head_and_Shoulders":("inverse_head_shoulders_flag", "bullish", "^", "green"),
        "Triangle_Breakout_Up":     ("triangle_breakout_up_flag",    "bullish", "^", "green"),
        "Triangle_Breakdown_Down":  ("triangle_breakdown_down_flag", "bearish", "v", "red"),
    }

    WINDOW = 80   # bars to show either side of the detection — increase for more context

    for pattern_name, (flag_col, direction, marker, color) in patterns.items():
        ...
        # Space examples out so windows don't overlap
        selected = []
        last_pos = -9999
        ...
        print(f"\n  {pattern_name}: {len(selected)} example(s) to plot")
        ...
        for n, detection_date in enumerate(selected, start=1):
            ...
            window_df = data.iloc[start:end + 1]

            fig, ax = plt.subplots(figsize=(14, 7))

            # --- Price line ---
            ax.plot(window_df.index, window_df["price"],
                    color="steelblue", linewidth=1.5, label="Price", zorder=2)

            # --- Pivot highs and lows ---
            ph = window_df[window_df["pivot_high_confirmed"] == 1]
            pl = window_df[window_df["pivot_low_confirmed"] == 1]
            ax.scatter(ph.index, ph["confirmed_pivot_high_price"],
                       marker="^", color="orange", s=100, zorder=4, label="Pivot High")
            ax.scatter(pl.index, pl["confirmed_pivot_low_price"],
                       marker="v", color="purple", s=100, zorder=4, label="Pivot Low")

            # --- Detection marker ---
            detection_price = float(data.loc[detection_date, "price"])
            ax.axvline(detection_date, color=color, linestyle="--",
                       linewidth=1.5, alpha=0.85, zorder=3)
            ax.scatter([detection_date], [detection_price],
                       marker=marker, color=color, s=200, zorder=5,
                       label=f"{pattern_name.replace('_', ' ')} detected")
            ...
            label_color = {"LONG": "green", "SHORT": "red", "NO TRADE": "gray"}.get(label_str, "black")

            # --- Titles and labels ---
            ax.set_title(
                f"{ticker}  |  {pattern_name.replace('_', ' ')}  ({direction.upper()})  "
                f"|  Date: {detection_date.date()}  |  Score: {score_val:.2f}",
            ...
            # --- Annotation box: future return and ML label ---
            annotation = (
                f"Forward {int(data.attrs.get('horizon', 10))}d Return: "
                f"{future_ret * 100:.1f}%\nML Label: {label_str}"
            ) if not np.isnan(future_ret) else f"ML Label: {label_str}"
            ...
            ax.annotate(
                ...
                va="bottom",
                color=label_color,
                bbox=dict(boxstyle="round,pad=0.4", facecolor="white", edgecolor=label_color, alpha=0.85),
            )

            plt.tight_layout()

            filepath = f"{output_prefix}_pattern_{pattern_name}_ex{n:02d}.png"
            save_or_show_plot(filepath, show_plots)
...
def run_named_chart_pattern_ml_framework(
    ...
    plot_equity_curves(train_bt, test_bt, ticker, output_prefix, show_plots=show_plots)
    plot_pattern_events(test_bt, ticker, output_prefix, show_plots=show_plots)

    print("\nGenerating individual pattern example charts...")
    plot_individual_pattern_examples(data, ticker, output_prefix, show_plots=show_plots, max_examples=5)
